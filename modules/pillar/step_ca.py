...
class StepCACLient:
  ...
  def initialize_auth_data(self):
    ...
    if self.provisioner:
      ...
      self.provisioner_password_file = self.master_setting_or_pillar_or_default('local_ca_password_file', 'local_ca_password_file', '{salt_step_dir}/config/password'.format(salt_step_dir=self.salt_step_dir))
  ...
  def list_to_options_string(self, option_name, list):
    return ' '.join(map(lambda entry: '--{option_name}="{entry}"'.format(entry=entry, option_name=option_name), list))
      ...
      for cert_type in self.cert_scopes:
        ...
        if cert_type in cert_pillar:
          ...
          for cert_name, cert_data in scope_cert_pillar.items():
            log.error("Processing cert_type {cert_type} with cert_name {cert_name} and cert_data {cert_data}".format(cert_type=cert_type, cert_name=cert_name, cert_data=cert_data))
            if 'cn' in cert_data:
